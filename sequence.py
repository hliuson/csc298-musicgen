...
# Custom small transformer model (Derek)
class TransformerSequencev2(pl.LightningModule):
    def __init__(self, num_heads=4, num_layers=3, embedding_size=128, hidden_size=256) -> None:
        super().__init__()
        # Autoregressive Transformer for next-step prediction
        # Inputs/outputs are (batch, seq_len, channels).
        ...
        #small transformer with ortho attention
        xformer_config = [
        {
            "reversible": False,  # Turn on to test the effect of using reversible layers
            "block_type": "encoder",
    ...
    def configure_optimizers(self):
        optimizer = optim.Adam(self.parameters(), lr=1e-5, weight_decay=1e-7)
        return optimizer
