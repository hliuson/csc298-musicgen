...
class MidiFormer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        pitch_oh = F.one_hot(pitch.long(), num_classes=128).float()
        dur_oh = F.one_hot(dur.long(), num_classes=128).float()

        pitch_loss = F.cross_entropy(self.pitch_head(y), pitch_oh)
        dur_loss = F.cross_entropy(self.dur_head(y), dur_oh)
        pos_loss = F.mse_loss(self.offset_head(y), pos.float().unsqueeze())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pitch_loss = F.cross_entropy(self.pitch_head(y), pitch_oh)
        dur_loss = F.cross_entropy(self.dur_head(y), dur_oh)
        pos_loss = F.mse_loss(self.offset_head(y), pos.unsqueeze())
        loss = pitch_loss + dur_loss + pos_loss
